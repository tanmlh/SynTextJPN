...
class RenderFont(object):
    ...
    def __init__(self, data_dir='data', lang="ENG"):
        ...
        modify
        """
        self.min_font_h = 3 #px : 0.6*12 ~ 7px <= actual minimum height
        self.max_font_h = 40 #px
        self.p_flat = 0.10
        ...
        self.text_source = TextSource(min_nchar=self.min_nchar,
                                      ...
                                      fn=osp.join(data_dir,'newsgroup/sentences.txt'),
                                      lang=lang)
    ...
    def place_text_given_bb(self, text_arrs, back_arr, bbs, bound_bb):
        areas = [-np.prod(ta.shape) for ta in text_arrs]
        order = np.argsort(areas)
        ...
        for i in order:
            ...
            if loc[0] + h >= H or loc[1] + w >= W:
                return back_arr,locs[:i],bbs[:i],order[:i]

            out_arr[loc[0]:loc[0]+h,loc[1]:loc[1]+w] += text_arrs[i]
    ...
    def render_sample(self,font,mask, given_bb=False, bound_bb=None):
        ...
        max_font_h = min(0.3*H, (1/f_asp)*W/(self.min_nchar+1))
        ...
        while i < self.max_shrink_trials and max_font_h > self.min_font_h:
            ...
            if given_bb:
                nline,nchar = self.get_nline_nchar([bound_h, bound_w],f_h,f_h*f_asp)
            else:
            ...
            if given_bb:
                text_mask, loc, bb, _ = self.place_text_given_bb([txt_arr], mask, [bb], [bound_bb])
...
class FontState(object):
    ...
    def get_font_size(self, font, font_size_px):
        ...
        """
        m = self.font_model[font.name.encode(encoding='utf-8')]
...
class TextSource(object):
    ...
    def __init__(self, min_nchar, fn, lang="ENG"):
        ...
        # parse Japanese text
            ...
            self.words = data.words(fn_chasen)
            self.sents = data.sents(fn_chasen)
            self.paras = data.paras(fn_chasen)
    ...
    def get_lines(self, nline, nword, nchar_max, f=0.35, niter=100):
        ...
        if len(lines) > 0 and type(lines[0]) == list:
            lines = lines[0]
